...
>
> This is the original **Product Requirements Document** for the Passenger Display System — the functional requirements (FR-AT-* tablet, FR-WD-* dashboard), MoSCoW prioritisation, and legal-requirements summary the product was designed and built against. It is frozen at v3.9 (May 2026).
>
> It is retained as the historical requirements record — **not** the authority for present-day design decisions. Current project state and remaining work live in `pds-planning/living/STATE.md`; the project's narrative history up to the July 2026 workflow migration is in `pds-planning/historical/PROJECT-HISTORY.md`. Where this document and the current implementation differ, that divergence is expected and the current implementation is authoritative.
...
Round-3 post-adversarial-review re-planning pass, item 3 of 4 (cross-document contradictions, missing details, and operational acknowledgements). Resolves `adversarial-review.md` findings 2, 3, 4, 6, 7, 8, 9, 10, 13, 14, 16, 17, 18, 19 — the non-audio-pipeline cluster.
- FR-AT-05: Fixed stale `FR-KM-03` cross-reference. The FR-KM- prefix does not exist anywhere in this document; the correct target for the "Deregister Device" action is **FR-AT-50**. Resolves round-3 finding 9.
- FR-AT-28: The "Audio not ready — syncing" screen now displays a second line directing the driver: **"Contact your fleet manager to verify route audio status."** Silent operation is non-compliant (Reg 12(1)(b)) so the gate remains a hard block; the new hint at least tells the driver who to ask. Resolves round-3 finding 16 (driver-side half).
- FR-AT-50: Credential wipe scope clarified to match FR-AT-04 terminal-failure parity — JWT, refresh token, **and** device secret are all wiped from EncryptedSharedPreferences (previously only JWT was named). An incomplete wipe would leave a stale device secret on disk that would obstruct re-pairing. Resolves round-3 finding 10.
- §1.4: Auto-deregistration is now a **compound condition** — 60-day `last_seen_at` silence AND ≥5 failed `recover-device` attempts in the trailing 60-day window. Distinguishes seasonal/in-repair silence (retains the `devices` row indefinitely; allows seamless return after months offline) from operator-decommissioned tablets that are being actively rejected by the backend. Resolves round-3 finding 13.
- §5.2: Added **Multi-tablet visual drift — acknowledged tolerance** paragraph. Two paired tablets on the same bus run independent GPS state machines that can drift slightly; this is acknowledged as a tolerance within the "sufficiently agreeing GPS fixes" assumption rather than a strict cross-tablet synchronisation guarantee. Single-bus pilot operation: drift is operationally invisible. Larger fleets: operator practice should include periodic verification. Resolves round-3 finding 14 (PRD half).
- §11.2 Dependencies: Added **Staging environment** as item 8 — two Supabase projects, two Vercel targets, six Sentry DSNs (one per surface per environment). Resolves round-3 finding 17.
- §11.2 Dependencies: Added **Operator practice — route edits before service** as item 9 — fleet managers should verify `audio_render_status = 'ok'` for affected routes before the next service. Pairs with FR-AT-28 hint above and addresses the driver-side blind spot. Resolves round-3 finding 16 (operator half).
- NFR-R-07: Strengthened `device_id` breadcrumb language — "`device_id` is a UUID identifier with no personal information embedded; it is permitted in Sentry breadcrumbs as a diagnostic correlation key." Removes ambiguity that the (deferred) Android CLAUDE.md Rule 17 contradicted. Resolves round-3 finding 3 (PRD half).
...
**Billable vs deregistered.** A device is **billable** if its `devices.last_seen_at` (maintained by the heartbeat mechanism, FR-AT-64 — not by route-sync recency alone) is within the last **30 days**. Devices that have been silent for more than 30 consecutive days are not invoiced. **Auto-deregistration** is a **compound condition**: a scheduled job automatically sets `devices.activation_state = 'inactive'` only when **both** (a) `last_seen_at` is older than 60 days, **AND** (b) the device has accumulated **at least 5 failed `recover-device` attempts in the trailing 60-day window** (tracked by `devices.recover_failure_count`; see Data Architecture §2.2 and §10). The compound condition distinguishes a tablet being **actively rejected** by the backend (operator decommissioned it, secret mismatch, hard-deleted row) — failures accumulate as the tablet tries to come back online — from a tablet that is **simply silent** (seasonal route paused, in-repair, OEM-throttled background, powered off). The silent case never accumulates failures, is **never** auto-deregistered, and retains its `devices` row indefinitely — re-pairing is always possible whenever the operator brings the tablet back online. The 30-day billing grace continues to apply independently of auto-deregistration and is purely a billing rule, not a lifecycle one — a silent seasonal tablet stops being billed at 30 days but keeps its row indefinitely. The 30-day billing window plus the 60-day-and-failures auto-deregistration gate together are the source of truth for both billing and lifecycle (see Data Architecture §10 for the schema-level rule and §4.3 for the `recover-device` failure-classification path that feeds the counter).
...
Once a device is paired, the setup screen is never shown again. The pairing details persist across app restarts and device reboots. The only way to return to the setup screen is via the admin "Deregister Device" action (FR-AT-50), which requires the admin PIN.
...
**Driver hint when audio is not ready.** Alongside the "Audio not ready — syncing" message, the app shows a second line: **"Contact your fleet manager to verify route audio status."** This directs the driver toward the only available recovery path — silent operation is non-compliant (Reg 12(1)(b)) so there is no driver-side override on the gate. The fleet manager has the dashboard view (FR-WD-13's persistent failed-render indicator and FR-WD-21's re-render action) to diagnose and recover; the driver has no equivalent. Without this hint a driver staring at a stuck route on a Monday morning has no actionable next step. Operator practice — verifying audio readiness on the dashboard before service runs — is the other half of this story; see §11.2 item 9 and Compliance Mapping Matrix Reg 12(1)(b).
...
The "Deregister device" admin action wipes all local route data and **all device credentials — JWT, refresh token, and device secret — from EncryptedSharedPreferences**, and returns the app to the first-run setup screen. This matches the credential-wipe scope of FR-AT-04 terminal-failure handling; an incomplete wipe (e.g. JWT only) would leave a stale device secret on disk that could not be reused with a different operator account but would obstruct re-pairing of the same physical tablet against a fresh `devices` row. The corresponding `devices` row in Supabase has `activation_state` set to `'inactive'`. This is the only way to re-pair a device with a different operator account.
...
**Multi-tablet visual drift — acknowledged tolerance.** Two tablets paired to the same physical bus run independent GPS state machines that each consume their own location fixes. Minor drift between the two tablets' "next stop" displays is possible because GPS chipsets fix at slightly different timings and accuracies; this is acknowledged as an acceptable tolerance within the "sufficiently agreeing GPS fixes" assumption rather than a strict cross-tablet synchronisation guarantee. The audio path is unaffected because audio is the responsibility of exactly one designated tablet per bus (`audio_enabled = true`) — the rear passenger hears the same audio bytes regardless of which screen they happen to be looking at. For the pilot deployment (single-bus-single-route operation) the drift is operationally invisible. For larger fleets, operator practice should include periodic verification that paired tablets on the same vehicle show consistent next-stop state during real-world operation; sustained disagreement indicates a GPS hardware issue on one tablet, not a software defect. See the Compliance Mapping Matrix Compliance Note 8 for the Reg 12(1)(b) framing of this tolerance.
...
**NFR-R-07: Crash Telemetry.** All three surfaces (Android tablet app, Next.js dashboard, Supabase Edge Functions) integrate Sentry (sentry.io) for crash and error reporting. The Android SDK captures unhandled exceptions, ANRs, and session crashes; the Next.js SDK captures client-side React errors, server-side rendering errors, and server-action errors; Edge Function SDKs report errors from `pair-device`, `recover-device`, `generate-pairing-code`, `enqueue-render-job`, `audio-render-worker`, `audio-cleanup-worker`, and `retry-admin-notification`. Breadcrumbs include diagnostic identifiers (`device_id`, `route_id`, `current_stop_index`); **`device_id` is a UUID identifier with no personal information embedded — it is permitted in Sentry breadcrumbs as a diagnostic correlation key** that bridges a captured crash record to the dashboard's per-device drill-down (FR-WD-23) and is essential for fleet-scale debugging. Breadcrumbs exclude operator names, passenger information, and any user-authored content; the system contains no passenger PII at all. The Sentry SDK fails open — if Sentry itself is unavailable, application behaviour is unaffected. Free-tier quota (5,000 errors/month) is expected to be sufficient at projected fleet size; see the Data Architecture §11.2 quota analysis (≈ 140/month expected steady state, ≈ 35× headroom). Three separate Sentry projects per environment (Android, Dashboard, Edge Functions × production + staging = six projects total) are provisioned at Stage 2/3 start; DSNs are delivered via environment variables (`SENTRY_DSN_ANDROID_PROD` / `_STAGING`, `SENTRY_DSN_DASHBOARD_PROD` / `_STAGING`, `SENTRY_DSN_EDGE_PROD` / `_STAGING`). See Data Architecture §11.2 and §12.
...
7. **Sentry (sentry.io)** — crash-reporting and error-tracking SDK for the Android tablet app, the Next.js dashboard, and Supabase Edge Functions. Free tier (5,000 errors/month) sufficient at projected fleet size (≈ 140 events/month expected steady state — see Data Architecture §11.2 quota analysis). **Three projects per environment** (Android, Dashboard, Edge Functions × production + staging = six projects total); DSN delivered via environment variable in each surface's build/runtime configuration, distinct per environment (`_PROD` / `_STAGING` suffixes). Sustained Sentry outages do not affect application behaviour — the SDK fails open. See Data Architecture §11.2 and NFR-R-07.
8. **Staging environment.** Stage 1 setup provisions **two Supabase projects** (production and staging) and **two Vercel deploy targets** (staging tracks Vercel preview deploys for every PR / branch push; production tracks the main-branch deploy). Both Supabase projects need the full setup (custom access token hook, pg_boss schema, scheduled functions, FCM credentials, Sentry DSNs distinct per environment). Google Cloud TTS may share one API key or use two separate keys (recommended: separate, for accounting clarity and per-environment quota isolation). See Data Architecture §12 for the full setup checklist and the order-dependencies note. Without staging, Vercel preview deploys would hit production data — a hard blocker on running design or schema iterations safely, and a risk that production state can be mutated by an in-progress branch.
9. **Operator practice — route edits before service.** Route edits that change stop ordering, stop names, route metadata, or route direction trigger a fresh audio render (FR-WD-20). The fleet manager **should verify on the dashboard route list** that `audio_render_status = 'ok'` for every affected route **before the next service runs**. Edits made shortly before a service should be treated as non-emergency actions — there is no driver-side recovery path for "audio not ready" (silent operation is non-compliant; see FR-AT-28 driver hint and Compliance Mapping Matrix Reg 12(1)(b)). This is an operator-discipline item, not a software-enforced gate; it is acknowledged here because the architecture relies on it. The dashboard surfaces the route-list red badge (FR-WD-13) and the persistent global failed-render indicator (FR-WD-13 v3.9 addition) to make the check easy and unmissable; FR-WD-21's re-render action is the operator's recovery mechanism if a render did fail.
