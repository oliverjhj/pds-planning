...
| 12(1)(b) | The audio and visual forms must be consistent with one another | Audio and visual content are generated from the same data source (the route and stop database), and audio is rendered server-side using a single consistent voice and rendering environment. All tablets in the operator's fleet — regardless of model, Android version, or manufacturer — play identical audio file bytes for any given announcement. Consistency is structural: there is no per-device variation in voice quality, accent, pronunciation, or audio rendering. Audio files are rendered once on the server at route-save time and distributed to every tablet via sync. **Multi-tablet audio designation:** in deployments with multiple tablets on one bus, exactly one tablet (the `audio_enabled = true` device, configured from the dashboard fleet view) produces audio output; all others run in display-only mode. This prevents overlapping announcements from multiple tablets playing the same file slightly out of sync, which would violate the consistency requirement. The visual display on all tablets remains consistent regardless of audio designation. The consistency guarantee covers: identical audio bytes, single audio source per bus, and audio-visual content parity across the fleet. | Software | FR-AT-28, FR-AT-32 |
...
| 13(4) | Audio must be within the frequency range of 300Hz to 3000Hz | Audio is rendered server-side using a specified TTS voice in a controlled, consistent rendering environment. The frequency characteristics of the rendered output are determined by the chosen TTS voice and rendering settings — both of which are selected and **verified by the developer before deployment** (not assumed). The output of the rendering pipeline can be measured with audio analysis tools during development to confirm compliance with the 300Hz–3000Hz range. **What the software guarantees:** the rendered MP3 files, as stored in Supabase Storage and distributed to tablets, contain speech audio with frequency content verified to fall within the regulatory range for the chosen voice and rendering configuration. **What the software does not guarantee:** that the operator's speaker hardware reproduces this frequency range without distortion or filtering — that is the operator's responsibility. The server-side rendering model is a substantially stronger basis for this compliance argument than on-device TTS (which varies by tablet model and cannot be centrally verified), but it remains contingent on the developer completing the voice verification step before production deployment. | Shared: Software generates and distributes rendered audio with verified frequency characteristics; Operator ensures speaker hardware fidelity | FR-AT-28 |
...
| Audio frequency (300Hz–3000Hz) | Renders audio server-side using a voice with verified frequency characteristics within the 300Hz–3000Hz range; rendering environment is controlled and testable at development time | Ensures speaker hardware does not distort or filter frequencies |
