> **HISTORICAL RECORD — REGULATORY REFERENCE, NOT DESIGN AUTHORITY**
>
> This is the **Compliance Mapping Matrix** for the Passenger Display System — the clause-by-clause mapping of the Public Service Vehicles (Accessible Information) Regulations 2023 (PSVAR) to the software features that satisfy each requirement. It is frozen at v3.9 (May 2026).
>
> Unlike the other historical documents, this matrix remains the primary **regulatory reference** for the project — where to look to understand which feature satisfies which regulation. It is still **not** the authority for present-day design decisions: where a feature has since evolved, the current implementation is authoritative and this matrix documents the original compliance intent. Current project state and remaining work live in `pds-planning/living/STATE.md`; the narrative history — including empirical compliance verifications such as the Reg 13(4) alert-frequency measurement — is in `pds-planning/historical/PROJECT-HISTORY.md`.
...
Round-3 post-adversarial-review re-planning pass, item 3 of 4 (cross-document contradictions, missing details, and operational acknowledgements). The Reg 12(1)(b) multi-tablet visual drift case (round-3 finding 14) is acknowledged as an operator-tolerance item in a new Compliance Note 8; a matching row is added to the Responsibility Boundary Summary.
- Compliance Note 8 (new): Reg 12(1)(b) multi-tablet visual drift on the same vehicle is acknowledged as an operator-tolerance item rather than a strict cross-tablet synchronisation guarantee. Drift is operationally invisible for the single-bus pilot; for larger fleets, operator practice should include periodic verification. Extends Note 4 (rearward-facing wheelchair spaces) with operator-practice guidance. Resolves round-3 finding 14.
- Responsibility Boundary Summary: Added "Multi-tablet visual cross-consistency on the same vehicle" row to make the software/operator boundary explicit. Sits directly under the existing "Audio-device designation in multi-tablet deployments" row so the two multi-tablet boundaries cluster together.

### v3.9 (May 2026 — round 3, item 2 of 4)
Round-3 post-adversarial-review re-planning pass, item 2 of 4 (audio pipeline tightening). Reg 13(4) row rewritten from "BLOCKING WORK" to "Verified" using the empirical results of the round-3 verification spike (`spike-records/round-3/findings-tts-frequency.md`). LINEAR16/WAV preservation noted as a strengthening of both Reg 13(4) and Reg 12(1)(b). All audio-file references in regulation rows updated from `.mp3` to `.wav` to reflect the end-to-end format switch.
- Reg 13(4): Voice-frequency verification status changed from "BLOCKING WORK — required pre-deployment" to "Verified" with empirical spike numbers quoted in-row (63.83 % in-band, 41.79 % formant zone, 66.67 % surviving 300 Hz hi-pass, smoothed upper cutoff 3 451 Hz, sibilant peak 3 287 Hz @ −17.8 dB, fundamental 128.9 Hz). Conditional re-verification trigger (voice change) preserved. LINEAR16/WAV pipeline cited as the structural reason the verified spectrum is the spectrum the tablet plays — no lossy compression in the path.
- Reg 12(1)(b): Strengthening sentence added — LINEAR16/WAV preserves audio bytes identically across the fleet with no encoder-driven variation.
- Reg 8(2), 10(2)(b), 11(2)(b), 11(5)(b): Bundled-asset filename references updated `.mp3` → `.wav` (`termination`, `diversion_start`, `hail_and_ride_start`, `hail_and_ride_end`).
- Responsibility Boundary Summary: "Audio frequency" row updated to reflect the verification is empirically complete and to cite the LINEAR16/WAV end-to-end preservation.

### v3.8 (May 2026 — item 3 of 4)
Round-2 post-adversarial-review re-planning pass, item 3 of 4 (missing-detail fixes and small bugs). No row content in this document required changes; the underlying PRD and Data-Architecture documents shifted in ways that do not affect any cited compliance argument. This entry exists for traceability so that anyone consulting the matrix is aware of the underlying changes.
- No matrix rows modified. A grep of the matrix for `devices.status`, `last_synced_with_return`, `stops_content_hash`, `rate_limit_attempts`, and `activation_state` returned no body matches; the rename of `devices.status → devices.activation_state` (Data-Architecture §2.2), the switch of FR-WD-12 divergence detection to `stops_content_hash` (PRD FR-WD-12; Data-Architecture §2.4, §4.4), the specification of the `rate_limit_attempts` table (Data-Architecture §2.9), the new FCM payload schema (Data-Architecture §7.6), the new `JOURNEY_AUTO_CLEARED` / `DIVERSION_REPLAYED` / `GMS_OVERRIDE_ACKNOWLEDGED` event types (Data-Architecture §5.4), the journey-state staleness recovery rules (PRD FR-AT-18 / Data-Architecture §5.5), the transient-vs-terminal `recover-device` failure classification (PRD FR-AT-04), the `audio_enabled = false` audio-download skip (PRD FR-AT-28 / FR-AT-55; Data-Architecture §7.2), the removal of scheduled hard-deletion language (Data-Architecture §10), the new FR-AT-67 GMS availability check (PRD), the 30-day/60-day billing/deregistration reconciliation (PRD §1.4; Data-Architecture §10), and the journey-events cleanup at journey start (PRD FR-AT-63 / FR-AT-11) all leave the matrix's existing compliance arguments intact.
- Note: Reg 12(1)(b) and the Responsibility Boundary Summary "Audio-device designation" row were already corrected to honest operator-responsibility framing in v3.8 item 2; that framing covers the multi-tablet audio-device case without further change.

### v3.8 (May 2026 — item 2 of 4)
Round-2 post-adversarial-review re-planning pass, item 2 of 4 (bug fixes and operational hardening). Reg 12(1)(b) rewritten to drop the dishonest claim that the software *prevents* overlapping multi-tablet audio; the row now describes the actual enforcement model (per-device flag + sensible defaults + soft dashboard warning), with the one-audio-tablet-per-bus invariant explicitly attributed to operator responsibility.
- Reg 12(1)(b): Multi-tablet clause rewritten. The software no longer claims to enforce "exactly one audio tablet per bus"; it provides the `audio_enabled` toggle, pairing defaults, and a zero-audio-devices soft warning (PRD FR-WD-22). The one-per-bus invariant is operator-configured. Misconfiguration is a compliance failure attributable to operator configuration, not software design.
- Responsibility Boundary Summary: New row added for "Audio-device designation in multi-tablet deployments" making the software/operator split explicit.
...
| 12(1)(b) | The audio and visual forms must be consistent with one another | Audio and visual content are generated from the same data source (the route and stop database), and audio is rendered server-side using a **single locked voice** (`en-GB-Neural2-B`, Google Cloud TTS — see Reg 13(4) and PRD FR-WD-20). All tablets in the operator's fleet — regardless of model, Android version, or manufacturer — play identical audio file bytes for any given announcement. Consistency is structural: there is no per-device variation in voice quality, accent, pronunciation, or audio rendering. The locked voice means consistency is invariant under any future per-operator or per-deployment configuration — there is no such configuration, by design. The only way to change the voice is a deliberate compliance event (re-running the Reg 13(4) verification and updating the code-level voice constant). Audio files are rendered server-side by the `audio-render-worker` job (Data Architecture §4.6) at route-save time and distributed to every tablet via sync. **Multi-tablet audio designation (operator-configured):** in deployments with multiple tablets on one bus, the software provides a per-device `audio_enabled` flag (configurable from the dashboard fleet view) and sensible pairing defaults — the first tablet paired to an operator account defaults to `audio_enabled = true`; subsequent tablets default to `false`. The dashboard also surfaces a soft warning when an operator's fleet contains zero `audio_enabled = true` devices (PRD FR-WD-22). The software **does not structurally enforce "exactly one audio device per bus"** — the `devices` schema has no concept of "vehicle" and no database constraint preventing multiple `audio_enabled = true` devices per operator. The operator is contractually responsible for configuring exactly one audio-on tablet per physical bus. Misconfiguration produces either silent operation (no audio-on tablet on a bus) or overlapping audio (multiple audio-on tablets on one bus); both are compliance failures attributable to operator configuration, not software design. See the Responsibility Boundary Summary row for "Audio-device designation" and PRD §5.2. The visual display on all tablets remains consistent regardless of audio designation. The consistency guarantee covers: identical audio bytes, single locked voice across all announcements and all operators, and audio-visual content parity across the fleet; the one-audio-tablet-per-bus invariant is operator-configured rather than software-enforced. **LINEAR16/WAV pipeline strengthening (v3.9):** the audio path is uncompressed LINEAR16 PCM (24 kHz mono, WAV container) end-to-end — TTS render → Storage → tablet playback — with no lossy compression stage. The bytes that leave the renderer are the bytes the tablet plays; there is no encoder-driven variation in voice quality between renders, between operators, or between tablets. This removes a degree of freedom that an MP3 (or any psychoacoustically-compressed) pipeline would have introduced. | Shared: Software provides the consistency-by-construction audio rendering, the per-device flag, and the soft warning; Operator configures the designation correctly per physical bus | FR-AT-28, FR-AT-32, FR-WD-20, FR-WD-22 |
...
| 13(4) | Audio must be within the frequency range of 300Hz to 3000Hz | Audio is rendered server-side using **Google Cloud Text-to-Speech, voice `en-GB-Neural2-B`** (see PRD FR-WD-20 and Data Architecture §4.6). The voice is locked — it is not configurable per operator, per route, or per deployment. The same voice is used both for route-specific files rendered by the `audio-render-worker` and for the bundled fixed-announcement files (termination, hail-and-ride start/end, diversion start/end) shipped in the APK. **Verified — see spike records.** The round-3 verification spike (`spike-records/round-3/findings-tts-frequency.md`, summarised in `spike-records/round-3/SPIKE-SUMMARY.md`) rendered a representative sample sentence ("The next stop is Newcastle upon Tyne Central Station. Please mind the gap between the bus and the kerb.") via `en-GB-Neural2-B` at LINEAR16 / 24 kHz mono, performed a Welch-PSD spectrum analysis (numpy 2.1.3 / scipy 1.14.1, 4096-bin Hann window, 50 % overlap, 5.86 Hz/bin resolution), and recorded these empirical results: **63.83 % of total spectral energy lies within the 300 Hz–3000 Hz band**; **41.79 % lies within the formant zone 500–2500 Hz** (the intelligibility-bearing region for F1/F2/F3); **66.67 % of energy survives a 300 Hz hi-pass** (the right metric for "would this voice still work through a PA limited to 300–3000 Hz reproduction"); the smoothed upper audible cutoff (≥ −20 dB) sits at **3 451 Hz**; the sibilant peak above 3 kHz is at **3 287 Hz @ −17.8 dB** relative to peak (moderate, inherent to human speech, acceptable per the sibilant tolerance discussed in this regulation row); the spectral peak at **128.9 Hz** is the male voice fundamental and carries little intelligibility — its presence below 300 Hz is expected and not a compliance issue. These numbers are the concrete evidence that satisfies Reg 13(4); the analysis is reproducible (procedure and pinned dependencies recorded in the spike). **Conditional re-verification trigger:** if the locked voice is ever changed from `en-GB-Neural2-B` (a Case 1 re-planning event per WORKFLOW.md), this verification must be re-run against the new voice; the row downgrade is conditional on the voice lock remaining in force. **LINEAR16/WAV preservation (v3.9):** the audio pipeline is uncompressed LINEAR16 PCM (24 kHz mono, WAV container) end-to-end. There is no MP3, Opus, or other psychoacoustically-compressed stage between the renderer's verified output and the tablet's speaker. The verified spectrum is therefore the spectrum the tablet plays — Storage stores the verified bytes, sync delivers the verified bytes, Android MediaPlayer plays the verified bytes. This is a structural strengthening: no encoder lies between the verification and production. **What the software guarantees:** every WAV file served to tablets contains speech audio produced by the verified voice in the verified rendering configuration; bytes are identical across the entire fleet (no per-device variation); no lossy compression alters the verified spectrum. **What the software does not guarantee:** that operator speaker hardware reproduces the 300 Hz–3000 Hz band without distortion or filtering — that remains the operator's responsibility. | Shared: Software produces verified-frequency audio (verification empirically complete); Operator ensures speaker hardware fidelity | FR-AT-28, FR-WD-20 |
...
| Audio-device designation in multi-tablet deployments | Provides per-device `audio_enabled` toggle in the dashboard fleet view; first-paired tablet defaults to audio-on, subsequent paired tablets default to audio-off; surfaces a soft warning when zero devices in the fleet are `audio_enabled = true` (PRD FR-WD-22). The software does **not** enforce a "one audio device per bus" constraint — `devices` has no concept of "vehicle" | Configures exactly one tablet per physical bus with `audio_enabled = true`; verifies designation matches the physical fleet layout; bears responsibility for compliance failures arising from misconfiguration (silent operation when no audio tablet is designated on a bus; overlapping audio when multiple are) |
| Multi-tablet visual cross-consistency on the same vehicle | Each tablet runs the spec-compliant GPS state machine independently; visual content (next stop, route progress, overlay timing) is generated identically from the same synced route data on each tablet — no per-device content variation. The software does **not** provide cross-tablet visual synchronisation: there is no shared journey state across tablets in the initial release, and tablets do not coordinate "next stop" timing with each other. Minor drift is an acknowledged tolerance within the "sufficiently agreeing GPS fixes" assumption (Compliance Note 8) | Periodically verifies (operationally, during real-world service) that paired tablets on the same vehicle show consistent next-stop state; sustained drift between two tablets on one bus is escalated as a hardware concern (GPS chipset on one tablet) and remediated by tablet replacement rather than logged as a software bug. For the single-bus pilot deployment, this verification is implicit in normal operation; for larger fleets it should be part of operator practice |
...
| Audio frequency (300Hz–3000Hz) | Renders audio server-side using Google Cloud TTS voice `en-GB-Neural2-B` (locked). Frequency content of the rendered output has been **verified empirically** in the round-3 spike (`spike-records/round-3/findings-tts-frequency.md`): 66.67 % of spectral energy survives a 300 Hz hi-pass, formant-zone share 41.79 %, smoothed upper audible cutoff 3 451 Hz. The LINEAR16/WAV pipeline preserves this verified spectrum end-to-end — no lossy compression at any stage. Re-verification is required only if the locked voice ever changes (which is itself a deliberate compliance event). | Ensures speaker hardware does not distort or filter frequencies |
...
### Note 8: Reg 12(1)(b) Multi-Tablet Visual Drift — Acknowledged Tolerance
Reg 12(1)(b) consistency across multiple tablets on the same vehicle is achieved by two distinct mechanisms with different guarantees. **Audio consistency is structural and strict**: the locked LINEAR16 audio source produces identical bytes across the fleet, and only the one tablet per bus with `audio_enabled = true` plays audio — there is no audio-side cross-tablet timing question to begin with. **Visual consistency is approximate**: each tablet runs an independent but spec-compliant GPS state machine on its own location fixes. Minor visual drift between two tablets on the same vehicle is possible due to GPS chipset variation and individual fix-timing differences; this is acknowledged as an **acceptable tolerance within the "sufficiently agreeing GPS fixes" assumption** rather than a strict cross-tablet synchronisation guarantee.

Extends Note 4 (rearward-facing wheelchair spaces, multi-tablet deployment) with explicit operator-practice guidance:
- **Pilot deployment (single-bus-single-route operation):** drift is operationally invisible — there is one bus and either one tablet or two co-located tablets running the same route at the same time, and rear-passenger experience is dominated by the (single) audio path, which is structurally identical across tablets.
- **Larger fleets:** operator practice should include periodic verification that paired tablets on the same vehicle show consistent next-stop state during real-world operation. Sustained disagreement indicates a GPS hardware issue on one tablet, not a software defect — both tablets are individually compliant; the disagreement is a per-device hardware quality signal. The remediation is hardware (swap the worse-performing tablet), not software.

This Note is the regulatory framing of the multi-tablet drift case acknowledged operationally in PRD §5.2.
